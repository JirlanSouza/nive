...
- `devtools` command, input schema, state snapshot, host state, panel config/state/message/effect reducer and effect runner, window spec, resource/operation view models, and `DevtoolStateCatalog`, `DevtoolStateHost`, and `DevtoolsApp` host trait contracts
...
- Keep app-domain fixture registration and the `DevtoolValue` state bridge in `app-gui` until domain values are wrapped or probe/fixture catalogs are abstracted; direct impls for `Vec<ProjectInfo>` and `Vec<Tag>` must stay with an app-owned trait. `DevtoolStateField` impls for `AsyncState<T>` and `OperationState<C>` also stay in `app-gui` because they reference `DevtoolValue` and app-specific error constructors.
- Keep concrete `DevtoolStateCatalog` and `DevtoolStateHost` derive impls and the `DevtoolStateField` trait in `app-gui`; the trait definitions for `DevtoolStateCatalog` and `DevtoolStateHost` live in `nive-runtime` and the derive macros generate `crate::dev::devtools::` paths that resolve through `app-gui` re-exports. `DevtoolValue` and `DevtoolStateField` remain in `app-gui` because `DevtoolValue` impls for `Vec<ProjectInfo>` and `Vec<Tag>` violate the orphan rule if the trait moves to a crate that neither owns `Vec` nor the domain types.
