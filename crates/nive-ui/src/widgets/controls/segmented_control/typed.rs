    ...
    Message: Clone + 'a,
{
    ...
    fn reserve_icon(&self) -> bool {
        self.options.iter().any(|option| option.icon.is_some())
    }

    fn metrics(&self, theme: Theme) -> ChoiceMetrics {
        ChoiceMetrics::for_theme(theme, self.size)
    }

    fn item_content(&self, index: usize, maximum_width: f32) -> Element<'a, Message> {
    ...
    fn reconciled_focus(&self, state: &SegmentedState) -> Option<usize> {
        state
            .focused_index
            .filter(|index| {
                self.options
    ...
    fn item_at(&self, state: &SegmentedState, layout: Layout<'_>, point: Point) -> Option<usize> {
    ...
    fn move_bounded(&self, state: &mut SegmentedState, direction: isize) -> Option<usize> {
