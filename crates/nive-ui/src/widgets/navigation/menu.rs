...
use self::widget::{MenuList, MenuSlot};
...
impl<'a, Message: Clone + 'a> Menu<'a, Message> {
    ...
    fn take_content(&mut self) -> Element<'a, Message> {
        ...
        MessageRelay::new(
            menu_content(std::mem::take(&mut self.entries)),
...
fn menu_content<'a, Message: Clone + 'a>(
    entries: Vec<MenuEntry<'a, Message>>,
    ...
    let mut content = Column::new()
        .padding(MENU_LIST_INSET)
        .width(Length::Fixed(MENU_MIN_WIDTH));
    let mut slots = Vec::with_capacity(entries.len());

    for entry in entries {
        let (eligible, activation, label) = match &entry {
            MenuEntry::Command(command) => {
                ...
                (activation.is_some(), activation, command.label.to_string())
            }
            MenuEntry::Checkbox(checkbox) => {
                ...
                (activation.is_some(), activation, checkbox.label.to_string())
            }
            MenuEntry::Radio(radio) => {
                ...
                (activation.is_some(), activation, radio.label.to_string())
            }
            MenuEntry::Submenu(submenu) => (!submenu.disabled, None, submenu.label.to_string()),
            MenuEntry::Separator => (false, None, String::new()),
        };
        slots.push(if matches!(&entry, MenuEntry::Separator) {
            MenuSlot::separator()
        } else {
            MenuSlot::row(eligible, activation.clone(), label)
        });
...
fn menu_row<'a, Message: Clone + 'a>(
    ...
    let mut content: Row<'a, MenuEvent<Message>, crate::theme::Theme, iced::Renderer> = Row::new();
    if reserve_choice {
    ...
    content = content
        .push(text_widget::with_typography::<iced::Renderer>(
            label,
    ...
    if let Some(trailing) = trailing {
        let trailing: Element<'a, MenuEvent<Message>> = match trailing {
            MenuTrailing::Shortcut(label) => text_widget::code_small(label).into(),
            MenuTrailing::Annotation(annotation) => text_widget::body_small(annotation).into(),
            MenuTrailing::Submenu => icon_widget::role(IconRole::NiveDisclosureRight)
    ...
    button(content)
        ...
        .width(Length::Fill)
        .on_press_maybe((!disabled).then_some(activation).flatten())
        .into()
}
